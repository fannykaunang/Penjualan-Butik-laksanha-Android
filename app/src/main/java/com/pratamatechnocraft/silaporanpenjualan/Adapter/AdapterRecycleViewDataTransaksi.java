...
//        this.type=type;
    ...
    public void onBindViewHolder(final ViewHolder holder, int position) {
        ...
        Float subTotal = Float.parseFloat(listItemTransaksi.getTOTAL_BAYAR().replace("Rp. ",""));

        holder.txtNoInvoiceTransaksi.setText(listItemTransaksi.getINVOICE_PENJUALAN());
        holder.txtTanggalTransaksi.setText(listItemTransaksi.getTANGGAL_PENJUALAN());
//        holder.txtTotalHargaTransaksi.setText(listItemTransaksi.getTOTAL_BAYAR());
        holder.txtTotalHargaTransaksi.setText("Rp. "+formatter.format(Double.parseDouble( String.valueOf(subTotal))));
...
//                }
                ...
                i.putExtra("kdTransaksi", listItemTransaksi.getINVOICE_PENJUALAN());
        ...
        protected FilterResults performFiltering(CharSequence charSequence) {
            ...
            if (charSequence == null || charSequence.length() == 0) {
                ...
                for (ListItemTransaksi itemTransaksi : listItemTransaksiFull) {
                    if (itemTransaksi.getINVOICE_PENJUALAN().toLowerCase().contains( filterPattern ) || itemTransaksi.getKETERANGAN().toLowerCase().contains( filterPattern ) || itemTransaksi.getKETERANGAN().toLowerCase().contains( filterPattern )) {
