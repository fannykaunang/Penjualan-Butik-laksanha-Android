...
public class InvoiceActivity extends AppCompatActivity {
    ...
    private static final String API_URL = "PenjualanTransaksi";
    Intent intent;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        myWebView.loadUrl(baseUrl+API_URL+ "/" + intent.getStringExtra( "kdTransaksi" ));
...
//        loadDetail();
//        Toast.makeText(InvoiceActivity.this, intent.getStringExtra( "kdTransaksi" ), Toast.LENGTH_SHORT).show();
...
//                            final JSONObject invoicedetail = new JSONObject(response);
                            ...
                            for (int i = 0; i < invoicedetail.length(); i++) {
                                ...
                                txtNoInvoiceDetailTransaksi.setText(jo.getString( "INVOICE_PENJUALAN" ));
                                txtNamaKasirDetailTransaksi.setText( jo.getString( "NAMA_USER" ) );
//                                if (jo.getString( "jenis_transaksi" ).equals( "0" )){
//                                    txtNoInvoiceDetailTransaksi.setText(  "#PL"+jo.getString( "kd_transaksi" ));
//                                }else{
//                                    txtNoInvoiceDetailTransaksi.setText(  "#PB"+jo.getString( "kd_transaksi" ));
//                                }


                                DecimalFormat formatter = new DecimalFormat("#,###,###");
                                Float subTotal = Float.parseFloat(jo.getString( "TOTAL_BAYAR" ));

//                                holder.txtSubTotalDetailTransaksi.setText("Rp. "+formatter.format(Double.parseDouble( String.valueOf(subTotal))));

                                txtTanggalTransaksiDetail.setText( jo.getString( "TANGGAL_PENJUALAN" ) + " " + jo.getString( "JAM_PENJUALAN" ) );
                                txtHargaTotalDetailInvoice.setText("Rp. "+formatter.format(Double.parseDouble( String.valueOf(subTotal))));
                                if (!jo.getString( "KETERANGAN" ).isEmpty() ){
                                    txtCatatanDetailTransaksi.setText( jo.getString( "KETERANGAN" ) );
                                }

                            }

//                            JSONArray detailinvoice = jo.getJSONArray("detailinvoice");
//                            for (int ii = 0; ii<detailinvoice.length(); ii++){
//                                JSONObject detailInvoiceObject = detailinvoice.getJSONObject( ii );
//                                String harga;
//                                if (jo.getString( "jenis_transaksi" ).equals( "0" )){
...
//                            }
                            ...
                            loadDetails(intent.getStringExtra( "kdTransaksi" ));
    ...
    private void loadDetails(String kdTransaksi){
        refreshInvoice.setRefreshing(true);
        StringRequest stringRequest = new StringRequest( Request.Method.GET, baseUrl+"PenjualanTransaksi/GetByID?id=" + kdTransaksi,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        try {
                            final JSONArray invoicedetails = new JSONArray(response);
                            for (int i = 0; i<invoicedetails.length(); i++){
                                JSONObject jo = (JSONObject) invoicedetails.get(i);
                                JSONObject detailInvoiceObject = invoicedetails.getJSONObject( i );
                                String harga;
...
//                                    harga=detailInvoiceObject.getString( "harga_beli_detail" );
//                                }
                                ListItemTransaksi listItemDetailTransaksi = new ListItemTransaksi(
                                        detailInvoiceObject.getString( "INVOICE_PENJUALAN"),
                                        detailInvoiceObject.getString( "NAMA_BARANG"),
                                        detailInvoiceObject.getString( "TANGGAL_PENJUALAN" ) + " " + detailInvoiceObject.getString( "JAM_PENJUALAN" ),
                                        detailInvoiceObject.getString( "JAM_PENJUALAN" ),
                                        Float.valueOf(detailInvoiceObject.getString("HARGA_JUAL")),
                                        detailInvoiceObject.getInt( "STOK" ),
                                        "Rp. " + detailInvoiceObject.getString( "TOTAL_BAYAR" ),
                                        detailInvoiceObject.getString( "KETERANGAN" ),
                                        detailInvoiceObject.getInt( "JUMLAH_BELI" )


                                );
//                                        detailInvoiceObject.getString( "NAMA_BARANG" ),
//                                        detailInvoiceObject.getString( "JUMLAH_BELI" ),
//                                        harga
                                Log.d( "TAG", "NAMA_BARANG: "+detailInvoiceObject.getString( "NAMA_BARANG" ) );
