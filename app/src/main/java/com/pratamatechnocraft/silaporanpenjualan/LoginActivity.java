...
public class LoginActivity extends AppCompatActivity {
    ...
    private void prosesLogin(final String user, final String pass){
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.POST, baseUrl + URL_LOGIN + "EMAIL=" + user + "&SANDI=" + pass,
                new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                try {
                    JSONObject jsonObject = new JSONObject(response);
                    String success = jsonObject.getString("LOGIN_RESULT");
                    if (success.equals("1")) {
//                        JSONObject data_user = jsonObject.getJSONObject("data_user");
                        String kd_user = jsonObject.getString("KODE_USER");
                        String level_user = jsonObject.getString( "LEVEL_USER" );

                        sessionManager.createSession(kd_user, level_user);
                ...
                new Response.ErrorListener() {
                    ...
                    public void onErrorResponse(VolleyError error) {
                        ...
                        loading.setVisibility(View.GONE);
                        btnLogin.setVisibility(View.VISIBLE);
                        Toast.makeText(LoginActivity.this, "Periksa koneksi & coba lagi" + error.toString(), Toast.LENGTH_SHORT).show();
                    }
                }
        )
        {
